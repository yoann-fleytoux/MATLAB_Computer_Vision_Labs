...
squareSize = 38;
for i = 1:numel(files)
   pics{i} = imread(files{i});
   %[imagePoints{i},boardSize{i}] = detectCheckerboardPoints(pics{i});
   %[worldPoints{i}] = generateCheckerboardPoints(boardSize{i},squareSize);
   %figure('name',strcat('proj_image_',num2str(4+i),' with points'));
   %imshow(pics{i}); hold on;
...
% B) Use the "showExtrinsics" function to represent the location of the pattern
% in the Camera coordinate system and to represent the camera in the Coordinates of the two patterns;
figure('name','Extrinsics of camera');
...
% 4. Calculate the maximum, minimum, median, mean and standard deviations. 
